...
const LeaderboardSkeleton = () => (
  <div className="flex flex-col animate-pulse">
    {[...Array(8)].map((_, i) => (
      <div key={i} className="flex items-center px-6 py-3.5 border-b border-indigo-900/40 relative">
        <div className="w-6 h-6 bg-white/10 rounded shrink-0" />
        <div className="w-[3.25rem] h-[3.25rem] rounded-full bg-white/10 ml-4 mr-4 shrink-0" />
...
export default function Leaderboard({ provider, onClose }: LeaderboardProps) {
  ...
  return (
    <article className="w-full h-full bg-[#141235] overflow-hidden relative flex flex-col font-sans">
      <div className="absolute inset-0 bg-gradient-to-b from-[#2e2671] to-[#141235] opacity-50 pointer-events-none" />
      {/* Confetti Background */}
      ...
      <div className="relative pt-12 sm:pt-16 px-4 sm:px-8 pb-6 flex flex-col shrink-0">
        {onClose && (
          <button 
            ...
            className="absolute top-4 left-4 w-9 h-9 flex items-center justify-center bg-transparent border border-white/20 hover:bg-white/10 rounded-full text-white transition-colors z-10"
          >
            <svg aria-hidden="true" className="w-5 h-5 pr-[1px]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15 19l-7-7 7-7" /></svg>
          </button>
        )}
        
        {/* Header content with better loading handling */}
        <div className={`transition-opacity duration-300 ${loading && scores.length === 0 ? 'opacity-50' : 'opacity-100'}`}>
          {scores.length > 0 && (
          ...
          <div className="flex items-center gap-2 mb-1.5 opacity-90">
            ...
            <span className="text-xs font-bold text-gray-100 tracking-wider">Leaderboard</span>
          </div>
          
          <h2 className="text-[26px] font-black text-white mb-1.5 leading-tight tracking-tight">
            {loading && scores.length === 0 ? 'Fetching Players...' : (scores.length > 0 && isWinner ? 'You are the winner!' : 'Top Players')}
          </h2>
          <p className="text-white font-black text-sm md:text-xl leading-snug drop-shadow-sm">
            {provider === 'playgames' ? 'Play Games' : provider.charAt(0).toUpperCase() + provider.slice(1)} Division<br/>
            <span className="opacity-90">{new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</span>
          </p>
        </div>
      </div>

      <div className="flex-1 bg-[#17153a] bg-opacity-95 overflow-y-auto custom-scrollbar border-t border-white/5 relative">
        {loading && scores.length === 0 ? (
          <LeaderboardSkeleton />
        ) : (
          <div className="flex flex-col">
            {scores.length === 0 && !loading ? (
               <p className="text-center text-white py-12 text-sm font-bold opacity-60">No scores yet. Be the first!</p>
            ) : (
                ...
                return (
                  <div key={entry.id} className={`flex items-center px-6 py-3.5 border-b border-indigo-900/40 relative ${isMe ? 'bg-indigo-900/30' : ''}`}>
                    ...
                    <div className="flex-1 min-w-0 pr-4 flex flex-col justify-center">
                      {isMe && (
                        <div className="text-[10px] text-white font-black tracking-widest mb-0.5 truncate uppercase bg-white/10 px-1 rounded inline-block w-fit">
                           You
                        </div>
                      )}
